...
void SectorManager::SendNearbyObjectsToPlayer(std::shared_ptr<Player> player)
{
    ...
    ForEachNearbyObject(basePlayer, [&](int nearbyId)
        {
...
void SectorManager::ForEachNearbyObject(std::shared_ptr<GameObject> object, std::function<void(int)> callback)
{
    if (!object) return;
    ...
    for (int y = centerY - 1; y <= centerY + 1; ++y)
    {
        for (int x = centerX - 1; x <= centerX + 1; ++x)
        {
            ...
            const auto& objects = _sectors[y][x].GetObjectIds();
            for (int playerId : objects)
            {
...
void SectorManager::ForEachNearbyPlayer(std::shared_ptr<GameObject> object, std::function<void(int)> callback)
{
    if (!object) return;
